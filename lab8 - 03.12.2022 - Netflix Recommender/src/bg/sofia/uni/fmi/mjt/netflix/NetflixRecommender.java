...
public class NetflixRecommender {
    ...
    public NetflixRecommender(Reader reader) {
        try (var bufferedReader = new BufferedReader(reader)) {
            contents = bufferedReader.lines().skip(1).map(Content::of).toList();
        } catch (IOException e) {
...
//        contents.stream().map(Content::genres).distinct();
        return null;
    }
    ...
    public Content getTheLongestMovie() {
        return contents.stream().filter(content -> content.type() == ContentType.MOVIE)
    ...
    public Map<ContentType, Set<Content>> groupContentByType() {
        return contents.stream().collect(Collectors.groupingBy(Content::type, Collectors.toSet()));
    }
    ...
    public List<Content> getTopNRatedContent(int n) {
        ...
        final int sensitivityThreshold = 10_000;
        final double averageRating = contents.stream().mapToDouble(Content::imdbScore).average().orElse(0.0);

        return contents.stream()
                .sorted(Comparator.comparingDouble(content -> {
                    double votesCount = content.imdbVotes();
                    double imdbScore = content.imdbScore();
                    return -((votesCount / (votesCount + sensitivityThreshold)) * imdbScore + (sensitivityThreshold /
                            (votesCount + sensitivityThreshold)) * averageRating);
                }))
                .limit(n).toList();
    }
    ...
    public List<Content> getSimilarContent(Content content) {
        return contents.stream().filter(obj -> obj.type().equals(content.type()))
                .sorted(Comparator.comparingLong(obj -> {
                    List<String> common = new ArrayList<>(obj.genres());
                    common.retainAll(content.genres());
                    return -common.size();
                }))
                .toList();
    }
